...
public abstract partial class SharedXenoArtifactSystem
{
    ...
    public Entity<XenoArtifactNodeComponent> CreateNode(Entity<XenoArtifactComponent> ent, XenoArchTriggerPrototype trigger, EntProtoId effect, int depth = 0)
    {
        ...
        nodeComponent.TriggerDifficulty = GetTriggerDifficulty(trigger); // Triad: stamp the rolled trigger's difficulty
    ...
    public void UpdateNodeResearchValue(Entity<XenoArtifactNodeComponent> node)
    {
        ...
        // Triad: floor the active-node multiplier. A freshly solved terminal node was worth exactly
        // zero upstream, which cancels the full-solve bonus at the point it should peak.
        var durabilityMultiplier = nonactiveNodes.Contains(node)
            ? MathF.Max(MinActiveDurabilityMultiplier, 1f - durabilityEffect)
            : 1f + durabilityEffect;
